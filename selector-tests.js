...
async function runSelectorTests() {
  ...
  TARGETS_TO_TEST.forEach(selector => {
    ...
    const components = container.querySelectorAll('k-button, k-badge, k-text, k-box, k-stack, k-row, k-grid, k-col, k-frame, k-section, k-container, k-loader');

    if (components.length > 0) {
      console.log(`✅ Target FOUND: "${selector}" (${components.length} component(s) mounted)`);
      components.forEach((comp, i) => {
        console.log(`   └─ [${i}] <${comp.tagName.toLowerCase()}>`);
      });
    } else {
      console.warn(`⚠️ Target FOUND: "${selector}", but NO components are mounted inside it.`);
    }
  });
