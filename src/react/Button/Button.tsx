...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  function Button(
    ...
    const classes = [
      ...
      // Utility tokens
      width ? `k-button--w-${width}` : '',
      height ? `k-button--h-${height}` : '',
      maxWidth ? `k-button--mw-${maxWidth}` : '',
      maxHeight ? `k-button--mh-${maxHeight}` : '',
      minWidth ? `k-button--mnw-${minWidth}` : '',
      minHeight ? `k-button--mnh-${minHeight}` : '',
      textAlign ? `k-button--txt-${textAlign}` : '',
      border ? `k-button--bdr${border === 'all' ? '' : '-none'}` : '',
      shadow ? `k-button--shd-${shadow}` : '',
      cursor ? `k-button--cur-${cursor}` : '',
      className ?? '',
    ]
      .filter(Boolean)
      .join(' ');
